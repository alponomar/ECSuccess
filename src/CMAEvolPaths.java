...
public class CMAEvolPaths {
    ...
    private double sigma, mueff, ccov, cc, mucov, cs;
    Matrix m, C, weights, pc, ps;

    public CMAEvolPaths(int newLambda, int newMu) {
        lambda = newLambda;
        mu = newMu;
...
//            weightsDouble[i] = 1.0 / mu;
        ...
        mucov = mueff;
        ccov = Math.min(mucov, n*n) / Math.pow(n, 2.0);
//        cc = 4.0 / n;
    ...
    public int makeStep(int currentCount) {
        ...
        Matrix pc2 = (m.minus(mold)).times(1.0 / sigma).times(Math.sqrt(cc * (2.0 - cc)*mueff));
        pc = pc1.plus(pc2);

        // 5. update covariance
        Matrix C1 = C.times(1.0 - ccov);
        Matrix C2 = pc.times(pc.transpose()).times(ccov / mucov);
