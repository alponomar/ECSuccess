...
public class RastriginEvaluationShifted implements ContestEvaluation
{
	// Evaluations budget
	...
	private double[] shift = {-5.0, 2.0, 0.0, 4.0, -2.0, 3.0, 1.0, -3.0, 2.0, 5.0};

	private static double ALPHA_ = 10;

	public RastriginEvaluationShifted()
	{
		best_ = 0;
		evaluations_ = 0;	
	}
	...
	{	
		...
		double sum = 0;
		for(int i=0; i<dim; i++) sum += (x[i]-shift[i])*(x[i]-shift[i]) - ALPHA_*Math.cos(2*Math.PI*(x[i]-shift[i]));
